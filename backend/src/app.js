...
function createApp() {
  ...
  function mountStaticRoute(route, folderName) {
    ...
    // Intercept HTML files to inject Cryptographic Nonces for Strict CSP
    app.get([route, `${route}/*.html`], async (req, res, next) => {
      let targetFile = req.path === route ? 'index.html' : req.path.replace(route, '');
      if (targetFile.startsWith('/')) targetFile = targetFile.substring(1);
      if (!targetFile) targetFile = 'index.html';
      
      const absolutePath = path.join(staticDir, targetFile);

      try {
        let htmlContent = fileCache.get(absolutePath);
        if (!htmlContent) {
          if (!fs.existsSync(absolutePath)) return next();
          htmlContent = await fs.promises.readFile(absolutePath, 'utf8');
          // Cache in all environments for low latency
          fileCache.set(absolutePath, htmlContent);
        }

        const nonce = res.locals.nonce; // Generated by securityHeaders
        
        // Inject nonce into all script tags
        let html = htmlContent.replace(/<script /g, `<script nonce="${nonce}" `);
